  ...
  return (req, res) => {
    ...
    if (h) {
      ...
      if (status === 'open') {
        ...
        if (adding) {
          logger.info(`adding ${prefix} server at ${uri}: ${[...map.keys()]}`);
          stats.gauge(gaugeName, map.size);
          createSet(fsServers === map ? setNameFs : setNameRtp, new Set(map.keys()));
        }
      }
      else {
        if (map.has(uri)) {
          map.delete(uri);
          logger.info(`removing feature server at ${uri}, leaving ${[...map.keys()]}`);
          stats.gauge(gaugeName, map.size);
          createSet(fsServers === map ? setNameFs : setNameRtp, new Set(map.keys()));
        }
      }
